...
export const useVideoGeneration = () => {
    ...
    useEffect(() => {
        // FIX: Explicitly cast the task to VideoTask to fix type errors.
        ...
        const poll = async () => {
            ...
            await Promise.all(tasksToPoll.map(async ([sourceUrl, task]) => {
                // FIX: Cast task to VideoTask to allow property access.
                ...
                try {
                    ...
                    if (updatedOp.done) {
                        ...
                        // FIX: Spread the typed task object to prevent spread errors.
                        // Replaced spread operator with Object.assign for robustness, as type inference issues
                        // could cause `typedTask` to be a non-object type, leading to a "Spread types may only be created from object types" error.
